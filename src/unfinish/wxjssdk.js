function callback(o, e) {
  if (!o.jWeixin) {
    var n,
      ...
      // i = window.document 获取当前窗口内的文档节点
      i = o.document,
      // t = window.document.title 获取文档的标题(即窗口或标签页顶部的标题栏)
      t = i.title,
    ...
    var I = !1,
      ...
      w = {
        ...
        // 是否需要检查WeixinJSBridge注入
          var t = !1 !== v.check;
          O(function () {
            // 当WeixinJSBridge注入完成后, 调用WeixinJSBridge.invoke("preVerifyJSAPI")验证配置是否正确, 并发送报告到微信服务器
            if (t)
              M(
                c.config,
                {
        ...
        // ready实际上是将回调函数push到全局的回调函数数组中, 等到全局的 "WeixinJSBridgeReady"事件触发后, 依次调用回调函数
        ready: function (e) {
  ...
  //(all) WeixinJSBridge.invoke的统一回调函数处理
  function A(e, n, i) {
    ...
    // 根据errMsg调用对应参数回调success, cancel, fail和complete
    var o = t.indexOf(":");
  ...
  // (convert) 将wx[api]映射成实际的api名称传递给WeixinJSBridge
  function C(e) {
    if (e) {
  ...
  // (once || onReady) WeixinJSBridge 注入完成
  function O(e) {
    // l = -1 != r.indexOf("micromessenger"),
    l &&
    // WeixinJSBridge是nactive端暴露给全局对象的方法
    // 如果WeixinJSBridge在window对象上挂载了, 直接执行回调方法
    // 否则, 注册WeixinJSBridgeReady事件监听函数, 由事件触发回调函数
      (o.WeixinJSBridge
        ? e()
        : i.addEventListener &&
